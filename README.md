## ⚠️ Текущий статус реализации (Work in Progress)

Проект находится в стадии частичной реализации. В связи с ограничениями по времени и объемом задач, полный функционал завершить не удалось.

### ✅ Реализовано
- [x] **User Management Service**:
  - Полностью спроектирован и написан самостоятельно (Clean Architecture / DDD).
  - Написаны Unit и Integration тесты (функциональны, создавались на раннем этапе разработки).
  - Все эндпоинты подготовлены и сохранены в папке `requests`.
- [x] **Базовая архитектура**: Настроены .NET Aspire и связь с БД.

### 🚧 В процессе / Частично реализовано
- [ ] **Product Management Service**:
  - Реализован только базовый каркас с использованием AI-ассистента (Cursor) для ускорения разработки.
  - Бизнес-логика не завершена.
- [ ] **Интеграционные события (RabbitMQ)**:
  - Код написан, но механизм не доведен до рабочего состояния (сообщения не обрабатываются корректно).

### ❗ Известные проблемы и запуск
- [!] **Swagger & JWT**: Авторизация через Swagger работает некорректно.
- [!] **Тестирование API**: Для проверки работоспособности используйте подготовленные `.http` файлы из папки `requests`, так как Swagger не полностью функционален.
